...
(()=>{
  ...
  function publicationActivityMarkup(){const latestEvent=records.reduce((m,r)=>String(r.event_at||'')>m?String(r.event_at||''):m,'').slice(0,10);const published=[...publications].filter(x=>x&&x.published&&x.date).sort((a,b)=>String(b.date).localeCompare(String(a.date)));if(!published.length)return '';const recent=published.filter(x=>String(x.date)>latestEvent).slice(0,8),display=recent.length?recent:published.slice(0,1);return `<section class="fcmo-activity-clock" data-fcmo-publication-activity="${esc(published[0].date)}"><div class="fcmo-clock-head"><span class="eyebrow">Latest public activity</span><h2>Publication clock</h2><p>${recent.length?'New verification and editorial activity is shown here without rewriting the original date of a development.':'The current published edition is shown separately from original development dates; no newer publication-only activity exists beyond the latest development clock.'}</p></div>${display.map(e=>`<a class="fcmo-clock-row" href="#/edition/${encodeURIComponent(e.date)}"><time>${esc(e.date)}</time><span><strong>${e.published?'Published edition':'Research snapshot'}</strong><small>${esc((e.preamble||[]).map(x=>x.text||'').join(' ').slice(0,180))}</small></span><b>${e.published?'ISSUED':'SNAPSHOT'} →</b></a>`).join('')}</section>`}
