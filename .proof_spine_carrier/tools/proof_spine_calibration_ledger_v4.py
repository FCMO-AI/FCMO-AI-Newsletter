...
def validate_case(case: Any) -> dict[str, Any]:
    ...
    if case.get("schema_version") != 4 or case.get("kind") != KIND or case.get("authority") != AUTHORITY:
    ...
    _strings(provenance.get("measurement_roots"), "gate.provenance.measurement_roots")

    if state == "EXECUTED":
        # Footnote [1]: a scoreable gate binds exact contract/evidence/decision bytes
        # plus the measurement paths that fed it. This prevents hand reconstructions
        # and makes later independence testable rather than rhetorical.
        ...
        # Footnote [2]: derived/reconstructed decisions remain useful evidence but may
        # not impersonate an executed decision by carrying execution-only digests.
    ...
    _strings(mechanism.get("measurement_roots"), "adjudication.mechanism.measurement_roots")
...
def classify(case: dict[str, Any]) -> dict[str, Any]:
    ...
    reasons: list[str] = []
    gate_roots = set(provenance["measurement_roots"])
    adjudication_roots = set(adjudication["mechanism"]["measurement_roots"])
    overlap = sorted(gate_roots & adjudication_roots)

    if actual == "UNKNOWN":
        reasons.append("UNKNOWN_ADJUDICATION")
    if adjudication["relation_to_spine"] != "INDEPENDENT_OF_SPINE":
        reasons.append("ADJUDICATION_NOT_INDEPENDENT_OF_SPINE")
    if overlap:
        # Footnote [3]: same-subject truth necessarily concerns the same world state,
        # but calibration still needs a distinct observation mechanism. Asking one
        # checker twice is consistency evidence, not specificity/sensitivity evidence.
        reasons.append("ADJUDICATION_REUSES_GATE_MEASUREMENT")
    ...
    return {
        ...
        "gate_measurement_roots": sorted(gate_roots),
        "adjudication_measurement_roots": sorted(adjudication_roots),
        "measurement_overlap": overlap,
...
def calibrate(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "events": events,
        "claim_boundary": (
            "Accuracy denominators require an executed byte-addressed Spine gate and later project-local adjudication independent of Spine through a disjoint observation mechanism. Re-running the same measurement path cannot manufacture sensitivity or specificity. Derived/reconstructed gates and same-measurement adjudications remain useful evidence but are UNSCORABLE. Zero denominators remain null."
        ),
    }
